...
void CLogDelOperate::DeleteBackupLogFile(const string & strLogPath, unsigned int keep_days, const string& strFileType)
{
	vector<string>  vFileInfoList;
	int count = 0;
	GetFileList(strLogPath, strFileType, keep_days, vFileInfoList, 100, count);
...
void CLogDelOperate::GetFileList(const string& strFullPath, const string& strFileType, unsigned int keep_Time, vector<string> & vFileInfoList, 
	int nMaxFileNum, int & nCurrentCount)
{
	string strCondition = strFullPath + "\\*" + strFileType; //后缀名构造
